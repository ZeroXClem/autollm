...
def clone_or_pull_repository(git_url: str, local_path: Path) -> None:
    ...
    try:
        ...
        logger.error(
            'GitPython is not installed. Please install the `git` package using `pip install gitpython==3.1.37` to use this feature. You can install it by running `pip install gitpython==3.1.37`.')
        raise
